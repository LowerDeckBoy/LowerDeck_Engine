...
{
	std::array<D3D12_CPU_DESCRIPTOR_HANDLE, RenderTargetsCount> rtvs{ m_RTVDescs.at(0).GetCPU(), m_RTVDescs.at(1).GetCPU(),m_RTVDescs.at(2).GetCPU(),m_RTVDescs.at(3).GetCPU(),m_RTVDescs.at(4).GetCPU() };
	...
	for (size_t i = 0; i < RenderTargetsCount; i++)
	{
		...
		D3D::g_CommandList.Get()->ClearRenderTargetView(rtvs.at(i), ClearColor.data(), 0, nullptr);
	}
...
void GBufferPass::CreateRenderTargets(D3D::D3D12Viewport* pViewport)
{
	...
	for (uint32_t i = 0; i < RenderTargetsCount; i++)
	{
		...
		D3D::D3D12Context::GetDeferredHeap()->Allocate(m_RTVDescs.at(i));
		...
		D3D::g_Device.Get()->CreateRenderTargetView(m_RenderTargets.at(i).Get(), &rtvDesc, m_RTVDescs.at(i).GetCPU());
