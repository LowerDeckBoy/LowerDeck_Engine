...
namespace D3D
{
	D3D12DepthBuffer::D3D12DepthBuffer(D3D12DescriptorHeap* pDepthHeap, D3D12Viewport Viewport, DXGI_FORMAT Format, bool bSRV)
	{
		m_Format = Format;
		Create(pDepthHeap, Viewport, bSRV);
	}
	...
	void D3D12DepthBuffer::Create(D3D12DescriptorHeap* pDepthHeap, D3D12Viewport Viewport, bool bSRV)
	{
		...
		const auto heapDesc{ CD3DX12_RESOURCE_DESC::Tex2D(m_Format,
													static_cast<uint64_t>(Viewport.Viewport().Width),
													static_cast<uint32_t>(Viewport.Viewport().Height),
													1, 0, 1, 0,
		...
		if (bSRV)
		{
			...
			//g_Device.Get()->CreateShaderResourceView(m_Resource.Get(), &srvDesc, m_DepthDescriptor.GetCPU());
		}
