...
void TextureUtility::CreateFromHDR(const std::string_view& Filepath, ID3D12Resource** ppResource)
{
	...
	desc.MipLevels	= static_cast<uint16_t>(scratchImage.GetMetadata().mipLevels);
...
void TextureUtility::CreateFromHDR(const std::string_view& Filepath, ID3D12Resource** ppResource, D3D::D3D12Descriptor& Descriptor, uint16_t MipLevels)
{
	if (utility::GetFileExtension(Filepath) != ".hdr")
	{
		// message here
	...
	assert(MipLevels > 0);
	scratchImage = GenerateMipChain(scratchImage, MipLevels, false);
	DirectX::TexMetadata metadata{ scratchImage.GetMetadata() };
